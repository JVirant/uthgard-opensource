		...
		public static void AddModelToObj(WavefrontObjFile objFile, NiFile model, Matrix worldMatrix, string[] filter, bool exclude)
		{
			...
			foreach (NiObject obj in model.ObjectsByRef.Values)
			{
				...
				if (shape != null)
				{
					...
					var textureCoordinates = new Vector2[data.Vertices.Length];
					...
					for (int i = 0; i < data.Vertices.Length; i++)
					{
						...
						if (data.UVSets.Length > 0)
							textureCoordinates[i] = new Vector2(data.UVSets[0][i].X, data.UVSets[0][i].Y);
					}
					...
					objFile.AddMesh(transformedVertices, data.UVSets.Length > 0 ? textureCoordinates : null, triangles.ToArray());
				}
