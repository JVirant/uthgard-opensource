...
namespace DaocClientLib.FileSystem
{
    /// <summary>
    ...
    internal class TinyMPK
    {
        ...
        #region Save/Load
        private static readonly Dictionary<string, TinyMPK> _cachedMPKs = new Dictionary<string, TinyMPK>();
        ...
        {
            if (_cachedMPKs.ContainsKey(file))
                return _cachedMPKs[file];
            var mpk = new TinyMPK();
            mpk.Load(file);
            _cachedMPKs.Add(file, mpk);
            return mpk;
        }

        public static void UnloadCachedMPKs()
        {
            _cachedMPKs.Clear();
        }
        ...
        public void Load(string file)
        {
            ...
            {
                throw new IOException("MPK Error: " + file, ex);
            }
        }
        ...
        public void Load(Stream stream)
        {
...

            {
                ...
                checksum = (uint)(header[0] + (header[1] << 8) + (header[2] << 16) + (header[3] << 24));
        ...
        {
            foreach (MPKFileEntry e in _entries)
            {
